# -*- coding: utf-8 -*-
# @Time    : 2018/11/16 21:33
# @Author  : Tianchiyue
# @File    : cnn_rnn_word.py
...
def cross_validation():
    model_name = 'cnn_rnn_word'
    saved_models_fp = 'saved_models/{}.hdf5'.format(model_name)
    index_list = load_data('../data/oof_index.pkl')
    word_padded_sequences,_, word_embedding_matrix, _, train_y = load_data('../data/processed.pkl')
    ...
    for fold_id, (train_index, test_index) in enumerate(index_list):
        rand_set()
        model = cnn_rnn(embedding_matrix, config)
